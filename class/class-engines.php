...
class WP_404_Auto_Redirect_Engines{
    ...
    function __construct(){
        ...
        // Engines: Register
        add_action('ar404/search/init',                             array($this, 'register_engines'),               1, 2);
        
        // Engines: Results
        add_filter('ar404/search/engine/default_fix_url',           array($this, 'engine_default_fix_url'),         1, 3);
        add_filter('ar404/search/engine/default_direct',            array($this, 'engine_default_direct'),          1, 3);
        add_filter('ar404/search/engine/default_post',              array($this, 'engine_default_post'),            1, 3);
        add_filter('ar404/search/engine/default_term',              array($this, 'engine_default_term'),            1, 3);
        add_filter('ar404/search/engine/default_post_fallback',     array($this, 'engine_default_post_fallback'),   1, 3);
    ...
    function register_engine($args){
        ...
        if(!$args['slug']){
            $args['slug'] = ar404_sanitize($args['name'], '_');
        }
        ...
        $engine = apply_filters('ar404/define/engine/' . $args['slug'], $args);
        if(!$engine){
            return;
        }
    ...
    function deregister_engine($slug){
        ...
        foreach($this->get_engines as $e => $engine){
            ...
            // Groups: Engine
            ar404_deregister_groups_engine($slug);
    ...
    function engine_default_direct($result, $query, $group){
        ...
        if(!$post_types = ar404_get_post_types($query['settings'])){
        ...
        if($found){
            ...
            // Post Meta 'ar404_no_redirect = 1'. Stop direct match
            elseif($query['settings']['rules']['exclude']['post_meta'] && get_post_meta($post->ID, 'ar404_no_redirect', true) == '1'){
    ...
    function engine_default_post($result, $query, $group){
        ...
        if(!ar404_is_empty($query['request']['wp_query']['post_type'])){
            ...
            if(!in_array($wp_query_post_type, ar404_get_post_types($query['settings']))){
                $wp_query = false;
            }
        ...
        // Run Search
        $search = ar404_search($args, $query);
        ...
        // Not found
            ...
            if($wp_query){
                ...
                set_query_var('ar404_engine_default_post', $wp_query_post_type);
    ...
    function engine_default_term($result, $query, $group){
        ...
        if(!ar404_is_empty($query['request']['wp_query']['taxonomy'])){
            ...
            if(!in_array($wp_query_taxonomy, ar404_get_taxonomies($query['settings']))){
                $wp_query = false;
            }
     ...
     * If the engine "WP Query: Post Type" failed, it will set a Query Var 'ar404_engine_wpq_pt'. This engine will use it to fallback to the Post Type Archive.
     *
     * @param $result
     * @param $query
     * @param $group
    ...
    function engine_default_post_fallback($result, $query, $group){
        ...
        // Query Var: 'ar404_engine_default_post' set by the engine "Default: WP Query" because it failed.
        $post_type = get_query_var('ar404_engine_default_post', false);
...
ar404()->engines = new WP_404_Auto_Redirect_Engines();

}


/**
 * ar404_register_engine
 *
 * @param $engine
 *
 * @return mixed
 */
function ar404_register_engine($engine){
	return ar404()->engines->register_engine($engine);
}


/**
 * ar404_deregister_engine
 *
 * @param $slug
 *
 * @return mixed
 */
function ar404_deregister_engine($slug){
	return ar404()->engines->deregister_engine($slug);
}


/**
 * ar404_get_engine_by_slug
 *
 * @param $slug
 *
 * @return mixed
 */
function ar404_get_engine_by_slug($slug){
	return ar404()->engines->get_engine_by_slug($slug);
}
 ...
 * @return mixed
 */
function ar404_engine_exists($slug){
	return ar404()->engines->engine_exists($slug);
}