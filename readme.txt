...
* Expose 'Auto-Redirect-404' headers on 404 pages. (Admin).
...
Auto Redirect 404 1.0 introduces the concept of engines and groups which let you customize your own searching & matching logic. The plugin comes with 5 engines and 1 default group out of the box!
...
Auto Redirect 404 is 100% compatible with all popular manual redirection plugins:
...
If you use one of them, but missed a manual redirection and a 404 is about to be displayed, Auto Redirect 404 will cover you.
...
add_filter('ar404/search/group', 'my_404_group_trigger', 10, 2);
...
add_filter('ar404/search/engine/my_engine', 'my_404_engine_definition', 10, 3);
...
`
add_action('ar404/search/init', 'my_404_manipulate_groups_and_engines');
function my_404_manipulate_groups_and_engines($query){
    ...
    ar404_reorder_group_engines(array(
    ...
    ar404_register_group_engines(array(
    ...
    ar404_deregister_engine('my_another_engine');
...
add_filter('ar404/search/group', 'my_404_group_trigger_forever', 10, 2);
...
add_filter('ar404/init', 'my_404_no_init', 10, 2);
...
add_action('ar404/after_redirect', 'my_404_after_redirect');
function my_404_after_redirect($query){
    ...
    wp_mail(
        'my@email.com', 
        'Auto Redirect 404: New redirection', 
...
3. Go to Settings > Auto Redirect 404 to change your settings.
...
= 1.1.1 =
* Security: Major hardening of SQL queries to prevent injection vulnerabilities
* Security: Implemented rigorous input sanitization and output escaping across all files including admin.php
* Performance: Optimized database queries with WHERE clauses and Object Caching
* Dev: Fixed debug mode warnings and modernized PHP syntax
...
* Improvement: Enhanced input sanitization for preview functionality
* Maintenance: Verified code modernization and cleaned up legacy prefixes
...
* Added: Expose 'Auto-Redirect-404' headers on 404 pages. (Admin).
...
* Added: `action('ar404/search/init', $query)`
* Added: `filter('ar404/search/group', $group, $query)`
* Added: `filter('ar404/search/query', $query)`
* Added: `filter('ar404/search/engine/{engine}', $result, $query, $group)`
* Added: `filter('ar404/search/results', $query)`
* Added: `filter('ar404/search/redirect', $redirect, $query)`
* Improvement: Core reworked from scratch for better extensibility.
* Removed: 'Hooks' tab

= 0.9.0.2 =
* Fix: Plugin priority set to 999 by debault
* Added: New Filter available `('ar404/init', $init, $request_uri)`
...
* Removed: Unnecessary filter `('ar404/settings', $settings)`
...
* Added: New Filter available `('ar404/settings', $settings)`
* Added: New Filter available `('ar404/redirect', $args, $settings)`
* Added: New Action available `('ar404/after_redirect', $args, $settings)`
* Added: New page header 'Auto-Redirect-404: true' on redirection
* Added: Exclude posts with the post meta: `ar404_no_redirect = 1` from possible redirections.
* Added: Exclude terms with the term meta: `ar404_no_redirect = 1` from possible redirections.
